...
const ProjectForm: React.FC<ProjectFormProps> = ({ project, onSubmit, onCancel }) => {
  const [formData, setFormData] = useState({
    ...
    category: '',
    responsible: '',
    status: 'not-started' as 'not-started' | 'in-progress' | 'completed' | 'delayed' | 'ongoing',
  });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <Select value={formData.status} onValueChange={(value: 'not-started' | 'in-progress' | 'completed' | 'delayed' | 'ongoing') => setFormData({ ...formData, status: value })}>
          <SelectTrigger>
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            ...
            <SelectItem value="ongoing">Sürekli</SelectItem>
          </SelectContent>
        </Select>
      </div>
